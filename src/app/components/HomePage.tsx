      ...
      <div className="p-5 flex flex-col sm:flex-row items-start sm:items-center gap-4">
        ...
        {reveal ? (
          <div className="flex-1 min-w-0">
            <div style={{ color: '#7eb89a', fontSize: '0.68rem', fontFamily: "'Twemoji Country Flags', 'DM Mono'", letterSpacing: '0.12em' }}>BOTE EN VIVO · PRONOSTICA PANTERA</div>
            <div style={{ fontFamily: "'Twemoji Country Flags', 'Oswald', sans-serif", color: '#f5a623', fontSize: '2.2rem', fontWeight: 700, lineHeight: 1.1, fontVariantNumeric: 'tabular-nums' }}>
              {money(main.amount, config.currency)}
            </div>
            <div className="flex flex-wrap gap-2 mt-1.5">
              {botes.slice(1).filter(b => b.amount > 0).map(b => (
                <span key={b.key} className="px-2 py-0.5 rounded-md" style={{ background: 'rgba(212,242,38,0.1)', color: '#d4f226', fontSize: '0.7rem', fontFamily: "'Twemoji Country Flags', 'DM Mono'" }}>
                  {b.label}: {money(b.amount, config.currency)}
                </span>
              ))}
              <span style={{ color: '#7eb89a', fontSize: '0.72rem', fontFamily: "'Twemoji Country Flags', 'Nunito Sans'" }}>
          ...
          <div className="flex-1 min-w-0">
            <div style={{ fontFamily: "'Twemoji Country Flags', 'Oswald', sans-serif", color: '#f5a623', fontSize: '1.5rem', fontWeight: 700, lineHeight: 1.1 }}>
        ...
        <div className="flex flex-col gap-2 flex-shrink-0 w-full sm:w-auto">
          <button onClick={() => onNavigate('my-predictions')} className="px-4 py-2 rounded-lg cursor-pointer" style={{ background: '#f5a623', color: '#062b1a', fontFamily: "'Twemoji Country Flags', 'Oswald', sans-serif", fontWeight: 700, fontSize: '0.82rem', letterSpacing: '0.04em' }}>
            JUGAR PRONOSTICA PANTERA
          </button>
          {config.rifaEnabled && (
            <button onClick={() => onNavigate('rifa')} className="flex items-center justify-center gap-1.5 px-4 py-2 rounded-lg cursor-pointer" style={{ border: '1px solid rgba(212,242,38,0.4)', color: '#d4f226', fontFamily: "'Twemoji Country Flags', 'Oswald', sans-serif", fontSize: '0.78rem', letterSpacing: '0.04em' }}>
    ...
    <div className="flex items-center gap-4">
      {[{ val: days, label: 'DÍAS' }, { val: hrs, label: 'HRS' }, { val: min, label: 'MIN' }, { val: sec, label: 'SEG' }].map(t => (
        <div key={t.label} className="text-center" style={{ minWidth: '2.6rem' }}>
          <div style={{ fontFamily: "'Twemoji Country Flags', 'Oswald', sans-serif", fontSize: '2.2rem', fontWeight: 700, color: '#f5a623', lineHeight: 1, fontVariantNumeric: 'tabular-nums' }}>
            {String(t.val).padStart(2, '0')}
          </div>
          <div style={{ fontSize: '0.6rem', color: '#7eb89a', letterSpacing: '0.15em', fontFamily: "'Twemoji Country Flags', 'DM Mono', monospace" }}>{t.label}</div>
        </div>
      ))}
    </div>
  );
    ...
    <div className="max-w-7xl mx-auto px-4 py-6">
      ...
      <div className="relative rounded-2xl overflow-hidden mb-6" style={{ minHeight: '280px', background: '#062b1a' }}>
        ...
        <div className="relative z-10 p-6 md:p-10 flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
          <div>
            <div style={{ fontFamily: "'Twemoji Country Flags', 'Oswald', sans-serif", color: '#7eb89a', fontSize: '0.85rem', letterSpacing: '0.2em', marginBottom: '6px' }}>
              BIENVENIDO, {userName.toUpperCase()}
            </div>
            <h1 style={{ fontFamily: "'Twemoji Country Flags', 'Oswald', sans-serif", color: '#ffffff', fontSize: '2.2rem', fontWeight: 700, lineHeight: 1.1, marginBottom: '12px' }}>
            ...
            <div className="flex flex-wrap gap-3 mt-5">
              ...
              <button onClick={() => onNavigate('results')} className="px-5 py-2.5 rounded-lg cursor-pointer transition-all"
                style={{ border: '1px solid rgba(245,166,35,0.4)', color: '#f5a623', fontFamily: "'Twemoji Country Flags', 'Oswald', sans-serif", fontSize: '0.9rem', letterSpacing: '0.05em' }}>
                VER PARTIDOS
              </button>
            </div>
          </div>
          <div className="flex flex-col items-center gap-2">
            <div style={{ color: '#7eb89a', fontSize: '0.7rem', letterSpacing: '0.15em', fontFamily: "'Twemoji Country Flags', 'DM Mono', monospace" }}>INICIA EN</div>
            <CountdownTimer target={config.lockDate} />
            <div className="w-full h-px mt-2" style={{ background: 'rgba(245,166,35,0.2)' }} />
            <div style={{ color: '#7eb89a', fontSize: '0.7rem', letterSpacing: '0.1em', fontFamily: "'Twemoji Country Flags', 'DM Mono', monospace" }}>JUEVES 11 JUN 2026</div>
          </div>
        </div>
      </div>
        ...
        <div className="rounded-xl overflow-hidden" style={{ background: '#0d5035', border: '1px solid rgba(255,255,255,0.07)' }}>
          <div className="px-5 py-3 flex items-center justify-between" style={{ borderBottom: '1px solid rgba(255,255,255,0.07)' }}>
            <div className="flex items-center gap-2">
              ...
              <span style={{ fontFamily: "'Twemoji Country Flags', 'Oswald', sans-serif", color: '#f5a623', fontSize: '0.9rem', letterSpacing: '0.08em' }}>PRÓXIMOS PARTIDOS</span>
            </div>
        ...
        <div className="rounded-xl overflow-hidden" style={{ background: '#0d5035', border: '1px solid rgba(255,255,255,0.07)' }}>
          <div className="px-5 py-3 flex items-center justify-between" style={{ borderBottom: '1px solid rgba(255,255,255,0.07)' }}>
            <div className="flex items-center gap-2">
              ...
              <span style={{ fontFamily: "'Twemoji Country Flags', 'Oswald', sans-serif", color: '#f5a623', fontSize: '0.9rem', letterSpacing: '0.08em' }}>MIS PRONÓSTICOS Y PUNTOS</span>
            </div>
            <button onClick={() => onNavigate('my-predictions')} style={{ color: '#7eb89a', fontSize: '0.72rem', fontFamily: "'Twemoji Country Flags', 'DM Mono', monospace" }}>gestionar →</button>
          </div>
          {predictions.length === 0 ? (
            <div className="px-5 py-8 text-center">
              ...
              <button onClick={() => onNavigate('my-predictions')} className="px-4 py-2 rounded-lg cursor-pointer" style={{ background: '#f5a623', color: '#062b1a', fontFamily: "'Twemoji Country Flags', 'Oswald', sans-serif", fontWeight: 700, fontSize: '0.82rem' }}>
                CREAR PRONÓSTICO
              </button>
            </div>
          ) : (
            <div>
              {predictions
                ...
                return (
                  <div key={p.id} className="px-5 py-3 flex items-center gap-3" style={{ borderTop: i > 0 ? '1px solid rgba(255,255,255,0.05)' : 'none' }}>
                    ...
                    <div className="flex-1 min-w-0">
                      <div style={{ color: '#e0f0e8', fontSize: '0.84rem', fontWeight: 600, fontFamily: "'Twemoji Country Flags', 'Nunito Sans'" }} className="truncate">{p.name}</div>
                      <div style={{ color: '#4a7d65', fontSize: '0.66rem', fontFamily: "'Twemoji Country Flags', 'DM Mono'" }}>{p.league === 'main' ? 'Principal' : leagueLabel(p.league)}</div>
                    </div>
                    <div style={{ fontFamily: "'Twemoji Country Flags', 'Oswald', sans-serif", fontSize: '1rem', color: '#f5a623', fontWeight: 700 }}>
              ...
              <div className="px-5 py-2.5" style={{ borderTop: '1px solid rgba(255,255,255,0.06)' }}>
                <p style={{ color: '#4a7d65', fontSize: '0.68rem', fontFamily: "'Twemoji Country Flags', 'DM Mono'" }}>La clasificación general es privada del organizador.</p>
              </div>
            </div>
          )}
        </div>
      ...
      <div className="rounded-xl p-5" style={{ background: '#0d5035', border: '1px solid rgba(255,255,255,0.07)' }}>
        <div className="flex items-center gap-2 mb-4">
          <Trophy size={14} style={{ color: '#f5a623' }} />
          <span style={{ fontFamily: "'Twemoji Country Flags', 'Oswald', sans-serif", color: '#f5a623', fontSize: '0.9rem', letterSpacing: '0.08em' }}>EQUIPOS CLASIFICADOS POR CONFEDERACIÓN</span>
        </div>
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-3">
          {qualifiedConfederations.map(conf => (
            <div key={conf.name} className="rounded-lg p-3 text-center" style={{ background: 'rgba(255,255,255,0.04)', border: `1px solid ${conf.color}25` }}>
              <div style={{ fontFamily: "'Twemoji Country Flags', 'Oswald', sans-serif", fontSize: '1.4rem', fontWeight: 700, color: conf.color, lineHeight: 1 }}>{conf.teams}</div>
              <div style={{ fontSize: '0.7rem', color: '#7eb89a', fontFamily: "'Twemoji Country Flags', 'DM Mono', monospace", letterSpacing: '0.05em', marginTop: '4px' }}>{conf.name}</div>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="mt-6">
        <div className="flex items-center gap-2 mb-4">
          <span style={{ fontFamily: "'Twemoji Country Flags', 'Oswald', sans-serif", color: '#f5a623', fontSize: '1.1rem', letterSpacing: '0.05em' }}>GRUPOS</span>
          <div className="flex-1 h-px" style={{ background: 'rgba(245,166,35,0.15)' }} />
          <button onClick={() => onNavigate('results')} style={{ color: '#7eb89a', fontSize: '0.75rem', fontFamily: "'Twemoji Country Flags', 'DM Mono', monospace" }}>ver todo →</button>
        </div>
          ...
          {GROUPS.map(group => (
            <div key={group.id} className="rounded-xl overflow-hidden" style={{ background: '#0d5035', border: '1px solid rgba(255,255,255,0.07)' }}>
              <div className="px-3 py-1.5" style={{ background: 'rgba(245,166,35,0.1)', borderBottom: '1px solid rgba(245,166,35,0.15)' }}>
                <span style={{ fontFamily: "'Twemoji Country Flags', 'Oswald', sans-serif", color: '#f5a623', fontSize: '0.8rem', letterSpacing: '0.1em' }}>GRUPO {group.id}</span>
              </div>
              <div className="p-2 flex flex-col gap-1">
                {group.teams.map(team => (
                  <div key={team.id} className="flex items-center gap-1.5">
                    <span className="text-sm">{team.flag}</span>
                    <span style={{ fontSize: '0.72rem', color: '#c0d8cc', fontFamily: "'Twemoji Country Flags', 'Nunito Sans', sans-serif" }}>{team.shortName}</span>
                  </div>
                ))}
              </div>
            </div>
