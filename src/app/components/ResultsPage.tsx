      ...
      <div className="flex gap-1 mb-6 p-1 rounded-xl" style={{ background: '#083524' }}>
        {tabs.map(tab => (
          <button
            ...
            style={{
              fontFamily: "'Twemoji Country Flags', 'Oswald', sans-serif",
              fontSize: '0.82rem',
          ...
          {byPhase.map(({ phase, matches }) => (
            <div key={phase} className="rounded-xl overflow-hidden" style={{ background: '#0d5035', border: '1px solid rgba(255,255,255,0.07)' }}>
              <div className="px-5 py-3 flex items-center justify-between" style={{ background: 'rgba(245,166,35,0.06)', borderBottom: '1px solid rgba(245,166,35,0.12)' }}>
                <span style={{ fontFamily: "'Twemoji Country Flags', 'Oswald', sans-serif", color: '#f5a623', fontSize: '0.85rem', letterSpacing: '0.1em' }}>
                  {phase.toUpperCase()}
                </span>
            ...
            return (
              <div key={group.id} className="rounded-xl overflow-hidden" style={{ background: '#0d5035', border: '1px solid rgba(255,255,255,0.07)' }}>
                <div className="px-5 py-3 flex items-center justify-between" style={{ background: 'rgba(245,166,35,0.08)', borderBottom: '1px solid rgba(245,166,35,0.15)' }}>
                  ...
                  {inProgress && (
                    <span className="px-2 py-0.5 rounded-full" style={{ background: 'rgba(212,242,38,0.1)', color: '#d4f226', fontSize: '0.6rem', fontFamily: "'Twemoji Country Flags', 'DM Mono'", letterSpacing: '0.08em' }}>
                      EN VIVO · PROVISIONAL
                    </span>
                  )}
                  {complete && (
                    <span className="px-2 py-0.5 rounded-full" style={{ background: 'rgba(74,222,128,0.1)', color: '#4ade80', fontSize: '0.6rem', fontFamily: "'Twemoji Country Flags', 'DM Mono'", letterSpacing: '0.08em' }}>
                      FINAL
                    </span>
                  )}
                </div>
                <div>
                  ...
                  <div className="px-5 py-2 grid gap-2" style={{ gridTemplateColumns: '1fr 60px 60px 60px 60px 60px', borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
                    {['EQUIPO', 'PJ', 'G', 'E', 'P', 'PTS'].map(h => (
                      <div key={h} className="text-right first:text-left" style={{ color: '#4a7d65', fontSize: '0.68rem', fontFamily: "'Twemoji Country Flags', 'DM Mono', monospace", letterSpacing: '0.08em' }}>
                        {h}
                      </div>
                    ))}
                  </div>
                  {rows.map((row, i) => (
                    ...
                    >
                      <div className="flex items-center gap-2">
                        <span style={{ color: i < 2 ? '#4ade80' : i === 2 ? '#d4f226' : '#4a7d65', fontSize: '0.75rem', fontFamily: "'Twemoji Country Flags', 'DM Mono'", minWidth: '16px' }}>{i + 1}</span>
                        <span className="text-lg">{row.team.flag}</span>
                        <span style={{ fontSize: '0.85rem', color: '#e0f0e8', fontFamily: "'Twemoji Country Flags', 'Nunito Sans', sans-serif" }}>{row.team.name}</span>
                      </div>
                      {[row.played, row.won, row.drawn, row.lost, row.points].map((v, j) => (
                        <div key={j} className="text-right" style={{ fontFamily: "'Twemoji Country Flags', 'DM Mono', monospace", color: j === 4 ? '#f5a623' : '#9cc4b2', fontSize: '0.85rem', fontWeight: j === 4 ? 700 : 400 }}>{v}</div>
                      ))}
                    </div>
                  ))}
                </div>
                <div className="px-5 py-2" style={{ background: 'rgba(0,0,0,0.1)' }}>
                  <span style={{ color: '#4a7d65', fontSize: '0.68rem', fontFamily: "'Twemoji Country Flags', 'DM Mono'" }}>
      ...
      {activeTab === 'clasificados' && (
        <div className="space-y-4">
          <div className="rounded-xl p-4 flex items-center gap-3" style={{ background: 'rgba(74,222,128,0.08)', border: '1px solid rgba(74,222,128,0.2)' }}>
            ...
            <span style={{ color: '#4ade80', fontSize: '0.85rem', fontFamily: "'Twemoji Country Flags', 'Nunito Sans', sans-serif" }}>
          ...
          {Object.entries(allConfTeams).map(([conf, teamIds]) => (
            <div key={conf} className="rounded-xl overflow-hidden" style={{ background: '#0d5035', border: '1px solid rgba(255,255,255,0.07)' }}>
              <div className="px-5 py-2.5 flex items-center gap-2" style={{ borderBottom: '1px solid rgba(255,255,255,0.06)', background: `${confColors[conf]}10` }}>
                ...
                <span style={{ fontFamily: "'Twemoji Country Flags', 'Oswald', sans-serif", color: confColors[conf], fontSize: '0.85rem', letterSpacing: '0.1em' }}>
              ...
              <div className="p-4 flex flex-wrap gap-2">
                {teamIds.map(id => {
                  ...
                  return (
                    ...
                    >
                      <span>{team.flag}</span>
                      <span style={{ fontSize: '0.8rem', color: '#c0d8cc', fontFamily: "'Twemoji Country Flags', 'Nunito Sans', sans-serif" }}>{team.name}</span>
